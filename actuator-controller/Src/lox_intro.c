...
//
//	This file contains methods for the LOX_INTRO
//  state.
//  LoxIntro has two verification Methods
...
uint32_t LoxIntro(struct StateVars *ctrl)
{
    ...
    if(VerifyState(ctrl->currentState) && VerifyState(ctrl->lastState))
    {
    	if((ctrl->currentState & LOX_INTRO) == LOX_INTRO){
    		...
    		Get_State_Disagree_Error_Msg(TxMessageBuffer1, LOX_INTRO, ctrl->currentState);
