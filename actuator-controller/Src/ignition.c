...
//  - Renamed to IGNITION 12/77/2020 [APJ]
//	This file contains methods for the Ignition
//  state.
...
uint32_t Ignition(struct StateVars *ctrl)
{
    ...
    if(VerifyState(ctrl->currentState) && VerifyState(ctrl->lastState))
    {
    	if((ctrl->currentState & IGNITION) == IGNITION){
    		...
    		Get_State_Disagree_Error_Msg(TxMessageBuffer1, IGNITION, ctrl->currentState);
