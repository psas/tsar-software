...
//  state.
//  LoxStart has two verification Methods
...
uint32_t LoxStart(struct StateVars *ctrl)
{
    ...
    {
    	if((ctrl->currentState & LOX_START) == LOX_START){
    		...
    		Get_State_Disagree_Error_Msg(TxMessageBuffer1, LOX_START, ctrl->currentState);
