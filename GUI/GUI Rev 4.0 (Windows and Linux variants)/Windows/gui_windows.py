...
def init_paramiko():
    ...
    try:
        ...
        global flag_1
        flag_1 = True # This flag is created with the objective that the Shutdown button can only 
        ...
        ###########################################################################
        # Thread Function that gets everything from the ssh stdout connection 
        ###########################################################################
        def writeall(sock):
            global sig_to_resend
            while True:
                data = ' '
                #Receive the max allowed of bytes and decoded to ASCII
                ...
                if '\x1b[K' in str(data):
                    data = data.replace('\x1b[K', '').replace('\r\x1b[1A', '')
                ...
                if 'Error:' in str(data):
                        index3 = data.find('Valve ')
                        # stract the problematic valve name
                        if not index3 == -1:
                            
                            problematic_valve = data[index3:index3+6].replace(' ', '')
                ...
                if 'Sensor' in data:
                    ...
                    for index in range(len(spl)):
                        if len(spl[index]) > 0:
                            #Only when the data packet is equal or bigger than 150 put it in the queue
                            # Doing this will help to control what items go into the queue 
                            # This Queue helps to store the data packets
